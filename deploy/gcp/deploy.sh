...
FEEDBACK_GCS_BUCKET="${FEEDBACK_GCS_BUCKET:-sta-feedback-3524d3}"
FEEDBACK_PUBLIC_BASE="${FEEDBACK_PUBLIC_BASE:-${SERVICE_URL:-}}"
...
if [[ -n "$FEEDBACK_GCS_BUCKET" ]]; then
  DEPLOY_ENV="${DEPLOY_ENV}|FEEDBACK_GCS_BUCKET=${FEEDBACK_GCS_BUCKET}"
fi
if [[ -n "$FEEDBACK_PUBLIC_BASE" ]]; then
  DEPLOY_ENV="${DEPLOY_ENV}|FEEDBACK_PUBLIC_BASE=${FEEDBACK_PUBLIC_BASE}"
fi
